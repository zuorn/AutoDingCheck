...
if not defined workday_type (
    ...
    :: 发送企业微信通知
    echo 正在发送错误通知到企业微信群...
    powershell -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $webhookUrl = '%webhook_url%'; $content = '❌ 错误：未获取到工作日类型，自动终止打卡流程！'; $body = @{ msgtype = 'text'; text = @{ content = $content } } | ConvertTo-Json -Depth 10; $utf8 = New-Object System.Text.UTF8Encoding $false; $bodyBytes = $utf8.GetBytes($body); try { Invoke-RestMethod -Uri $webhookUrl -Method Post -Body $bodyBytes -ContentType 'application/json; charset=utf-8' | Out-Null } catch { }"
    exit
) else (
...
set /a "countdown=%random_delay%"
:countdown_loop
if !countdown! LEQ 0 goto :countdown_end
...
if errorlevel 1 (
    ...
    :: 发送企业微信通知
    echo 正在发送错误通知到企业微信群...
    powershell -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $webhookUrl = '%webhook_url%'; $content = '❌ ADB设备检查失败错误：未检测到ADB环境或设备未连接！请确认：1. ADB已配置环境变量或脚本中使用ADB绝对路径2. 手机已开启USB调试并通过USB/无线连接电脑'; $body = @{ msgtype = 'text'; text = @{ content = $content } } | ConvertTo-Json -Depth 10; $utf8 = New-Object System.Text.UTF8Encoding $false; $bodyBytes = $utf8.GetBytes($body); try { Invoke-RestMethod -Uri $webhookUrl -Method Post -Body $bodyBytes -ContentType 'application/json; charset=utf-8' | Out-Null } catch { }"
    pause
    exit
)
...
if not defined device_online (
    ...
    :: 发送企业微信通知
    echo 正在发送错误通知到企业微信群...
    powershell -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $webhookUrl = '%webhook_url%'; $content = '❌ ADB设备检查失败错误：设备未授权（unauthorized）或离线（offline）！请在手机上确认USB调试授权，或重新插拔数据线。'; $body = @{ msgtype = 'text'; text = @{ content = $content } } | ConvertTo-Json -Depth 10; $utf8 = New-Object System.Text.UTF8Encoding $false; $bodyBytes = $utf8.GetBytes($body); try { Invoke-RestMethod -Uri $webhookUrl -Method Post -Body $bodyBytes -ContentType 'application/json; charset=utf-8' | Out-Null } catch { }"
    pause
    exit
)
echo ✅ 设备连接正常！
...
if errorlevel 1 (                           
    ...
    :: 发送企业微信通知
    echo 正在发送错误通知到企业微信群...
    powershell -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $webhookUrl = '%webhook_url%'; $content = '❌ 错误：钉钉启动失败！可能原因：钉钉包名/Activity路径错误，或手机未安装钉钉。'; $body = @{ msgtype = 'text'; text = @{ content = $content } } | ConvertTo-Json -Depth 10; $utf8 = New-Object System.Text.UTF8Encoding $false; $bodyBytes = $utf8.GetBytes($body); try { Invoke-RestMethod -Uri $webhookUrl -Method Post -Body $bodyBytes -ContentType 'application/json; charset=utf-8' | Out-Null } catch { }"
    pause
    exit
)
...
:: 发送钉钉消息打卡成功
echo 发送钉钉消息打卡成功...
powershell -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $webhookUrl = '%webhook_url%'; $content = '✅打卡截图凭证：'; $body = @{ msgtype = 'text'; text = @{ content = $content } } | ConvertTo-Json -Depth 10; $utf8 = New-Object System.Text.UTF8Encoding $false; $bodyBytes = $utf8.GetBytes($body); try { Invoke-RestMethod -Uri $webhookUrl -Method Post -Body $bodyBytes -ContentType 'application/json; charset=utf-8' | Out-Null } catch { }"
